...
import { Search, Plus, Check, ChevronRight, Sparkles, ArrowLeft, X, Clock, Zap, WashingMachine, ChevronsDownUp, ChevronsUpDown, Share2, GripVertical, Save, RotateCcw, Trash2, Copy, Brain, BarChart3, Timer, RefreshCw, Thermometer, CloudRain, Eye, Loader, Shirt, DoorOpen, Edit3, BatteryCharging, RotateCw, Lock, Unlock } from "lucide-react";
...
import { migrateTrip, migrateTemplate } from "./lib/migrations";
...
export default function PackPal() {
  ...
  // ═══ OUTFIT BUILDER ═══ (Outfits batch: shortlist + day plan; outfits themselves live in savedOutfits)
  if (outfitMode && activeTrip) {
    return <><OutfitBuilder trip={activeTrip} savedOutfits={savedOutfits} setSavedOutfits={setSavedOutfits}
      ...
      onSave={(occasions, dayNames, syncToList, dayEmojis, outfitIds) => {
        ...
        if (syncToList) {
          // Pack every piece of every outfit on this trip: the day plan plus the shortlist (assigned to a day or not).
          const shortlist = (planFields.outfitIds).map(oid => (savedOutfits || []).find(o => o.id === oid)).filter(Boolean);
          const outfitItems = collectOutfitItems(occasions, shortlist);
          ...
          setTrips(p => p.map(t => {
            ...
            return { ...t, ...planFields, items: [...nonOutfit, ...kept, ...brandNew] };
          }));
        } else {
          setTrips(p => p.map(t => t.id === activeTrip.id ? { ...t, ...planFields } : t));
        }
      }} /><CelebrationLayer /></>;
  }
      ...
      <div style={{ padding: "0 20px 32px" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(2,1fr)", gap: 12 }}>
          {[
            ...
            { label: "My Outfits", sub: savedOutfits.length ? `${savedOutfits.length} saved outfit${savedOutfits.length === 1 ? "" : "s"}` : "Your closet", icon: <Shirt size={20} />, act: () => setView("closet"), col: "#C47EAA" },
