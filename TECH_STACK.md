...
| Files | **Cloud Storage for Firebase** | Outfit photos only (`outfits/{uid}/{outfitId}.jpg` + a ~480 px `{outfitId}_card.jpg` for the cards, owner-only rules); the state doc keeps the URL + a 2 KB thumb. |
...
Storage: outfits/{uid}/{outfitId}.jpg (+ _card.jpg)      // client rw (own), images < 3 MB — storage.rules
```
