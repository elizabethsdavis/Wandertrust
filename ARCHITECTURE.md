...
`icon-512.png`, `favicon-32.png` (all generated by `scripts/make-icons.py`
from `scripts/icon-src/`) and `manifest.webmanifest`. No SVG favicon on
purpose — iOS would pick it over the touch icon for the Home Screen.
...
items under that category id in `CORE` (`data/catalog.js`). Its label and emoji
are only defaults — the user can override both from the template editor
(`lib/categories.js`), so read them through `resolveCategories()`. Moving *existing*
